...
const socialLinks = [
  {
    href: 'https://github.com/ZhengLong9',
  ...
  {
    href: 'https://www.linkedin.com/in/soo-zhenglong-8ab5a133a/',
    icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="container lg:grid lg:grid-cols-2 lg:items-stretch">
        <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
          ...
          <p className="text-zinc-400 mt-3 mb-8 max-w-[50ch] lg:max-w-[30ch] reveal-up">
            Whether it’s about projects, ideas, or advice—I’d be glad to hear from you.
          </p>


          <div className="flex items-center gap-2 mt-auto">
            {socialLinks.map(({ href, icon }, key) => <a href={href} className='w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/50 rounded-lg transition-[background-color, color] hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80 reveal-up' key={key} target="_blank">{icon}</a>)}
          </div>
        </div>
        <form action="https://getform.io/f/aqoeyypa" className="xl:pl-10 2xl:pl-20 " method='POST'>
          <div className="md:grid md:items-center md:grid-cols-2 md:gap-2 ">
            <div className="mb-4">
              <label htmlFor="name" className="label reveal-up">Name</label>
              <input type="text" name="name" id="name" className="text-field reveal-up" autoComplete='name' required placeholder="Name" />
            </div>

            <div className="mb-4">
              <label htmlFor="email" className="label reveal-up">Email</label>
              <input type="email" name="email" id="email" className="text-field reveal-up" autoComplete='email' required placeholder="email.com" />
            </div>
          </div>
          <div className="mb-4">
            <label htmlFor="message" className="label reveal-up">Message</label>
            <textarea name='message' placeholder='Leave me a message' id='message' className='text-field resize-y min-h-32 max-h-80 reveal-up'>
            </textarea>
          </div>
